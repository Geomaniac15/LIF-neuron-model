...
CURRENT-BASED SYNAPSE VARIANT (stable backward pass).

WHY THIS VARIANT EXISTS
-----------------------
The conductance-synapse trainer (train_memory_bptt.py) is mathematically
correct (its finite-difference gradient check passes at ~1e-10) but its
through-time adjoint is EXPONENTIALLY DIVERGENT: the synaptic current
Isyn = -ge*(V - E_exc) - gi*(V - E_inh) is voltage-dependent, so the backward
pass picks up a multiplicative (V - E) ~ 65-80 mV factor every step AND a direct
gV->gV feedback term d(Isyn)/dV. That self-amplifying loop makes the per-step
adjoint norm fill whatever clip ceiling you set (1e3 -> 13k, 1e7 -> 1e8), so the
gradient reaching W is pure clipped noise and recall MSE never leaves chance
(1/3). See diagnostics/bptt_rootcause.png.

THE FIX
-------
Use CURRENT-BASED synapses for the trained model:
    Isyn = g_e * ge - g_i * gi          (no voltage dependence)
ge, gi are still per-step decaying traces driven by pre-synaptic spikes through
W, but the current they inject no longer scales with (V - E). This kills the
d(Isyn)/dV self-feedback term entirely and removes the compounding (V - E)
amplifier from the conductance adjoints, so the backward recursion stays
bounded. g_e, g_i are constant gains chosen so the resting-state recurrent drive
roughly matches the old conductance model (|V - E_exc| ~ 65, |V - E_inh| ~ 15).

IMPORTANT: this changes the forward dynamics, so the benchmark (reservoir_
capacity.py) MUST be run with the same current-based synapse model for the saved
W to transfer. The conductance benchmark will NOT measure this network correctly.

HOW (surrogate gradient)
---
The discontinuous spike s = H(V - V_th) is given a smooth surrogate derivative
phi'(V - V_th) (fast sigmoid, SuperSpike / Zenke & Ganguli 2018 style) so that
gradients flow through spikes.
...
# ----------------------------------------------------------------------------
# Firing-rate auto-calibration
# ----------------------------------------------------------------------------
def calibrate_drive(W, params, args):
    '''Binary-search the mean background current params['I_ext'] so the frozen-W
    network settles at a target firing rate, BEFORE training starts.

    Why: with current-based synapses the rate-vs-drive curve is steep and
    strongly N-dependent (the same background that gives ~13 Hz at N=300 leaves
    N=2000 silent). A silent or saturated network carries no trainable memory,
    so we recentre the per-neuron I_ext (preserving its spread) to land the
    steady-state population rate in [rate_lo, rate_hi].

    Uses the SAME forward_window dynamics as training (current synapses,
    adaptation, refractory, reset), so the calibrated rate is exactly what the
    trainer will see. Mutates and returns params with the new I_ext.
    '''
    N = params['N']; n_exc = params['n_exc']; dt = params['dt']
    tau_m = params['tau_m']; inv_tau_m = 1.0 / tau_m
    R = params['R']; V_rest = params['V_rest']; V_reset = params['V_reset']
    V_th = params['V_th']; tau_syn = params['tau_syn']
    syn_gain_exc = args.syn_gain_exc; syn_gain_inh = args.syn_gain_inh
    adapt_on = params['adapt_b'] > 0.0; adapt_b = params['adapt_b']
    ...
    base_centered = params['I_ext'] - params['I_ext'].mean()  # keep per-neuron spread
    n_steps = args.calib_steps
    # Only count spikes in the SETTLED tail: spike-frequency adaptation (tau_w)
    # makes the early transient fire far faster than the steady state, so
    # averaging over the whole window overestimates the rate the trainer sees
    # (which runs long past the transient). Measure the last `tail` steps only.
    warm = int(n_steps * args.calib_warm_frac)
    tail = max(1, n_steps - warm)
    tail_s = tail * dt / 1000.0

    def _measure_rate(I_mean):
        # Lightweight frozen-W forward (no taping, sparse spike scatter) that
        # reproduces forward_window's current-based dynamics exactly, including
        # adaptation, so the calibrated rate is the ADAPTED steady-state rate.
        I_ext = base_centered + I_mean
        V = np.full(N, -65.0); ge = np.zeros(N); gi = np.zeros(N)
        wad = np.zeros(N); refr = np.zeros(N)
        n_spk = 0
        for _t in range(n_steps):
            gate = (refr <= 0.0).astype(np.float64) if use_refr else 1.0
            Isyn = syn_gain_exc * ge - syn_gain_inh * gi
            Vh = V + inv_tau_m * (-(V - V_rest) + R * (I_ext + Isyn) - wad) * dt
            s = ((Vh >= V_th).astype(np.float64)) * gate
            V = Vh - (Vh - V_reset) * s
            if adapt_on:
                wad = wad * adapt_decay + adapt_b * s
            fe = np.nonzero(s * exc_mask)[0]
            fi = np.nonzero(s * inh_mask)[0]
            ge = (ge + (W[fe].sum(axis=0) if fe.size else 0.0)) * syn_decay
            gi = (gi - (W[fi].sum(axis=0) if fi.size else 0.0)) * syn_decay
            if use_refr:
                refr -= dt; refr[s > 0.5] = args.refractory
            if _t >= warm:
                n_spk += int(s.sum())
        return n_spk / (N * tail_s)

    lo, hi = args.calib_lo_I, args.calib_hi_I
    mid = 0.5 * (lo + hi); rate = float('nan')
    for _ in range(args.calib_iters):
        mid = 0.5 * (lo + hi)
        rate = _measure_rate(mid)
        if rate < args.calib_rate_lo:
            lo = mid
        elif rate > args.calib_rate_hi:
            hi = mid
        else:
            break
    params['I_ext'] = base_centered + mid
    print(f'[calib] background I_ext mean={mid:.3f}  steady rate={rate:.1f}Hz '
          f'(target {args.calib_rate_lo:.0f}-{args.calib_rate_hi:.0f}Hz, '
          f'settled tail of {n_steps} steps)', flush=True)
    return params
...
def forward_window(W, W_out, carry, drive_win, targets, readout_idx,
    ...
    for t in range(n_sim):
        ...
        Isyn = syn_gain_exc * ge - syn_gain_inh * gi   # current-based: no V dep
...
def backward_window(W, tape, readout_grads, g_rate, params, args, smooth):
    ...
    for t in range(n_sim - 1, -1, -1):
        ...
        # The reset-magnitude gradient gs += gV*-(Vh - V_reset) carries a ~15 mV
        # gain; combined with a near-1 surrogate for neurons close to threshold it
        # is the dominant per-step amplifier that makes the adjoint explode.
        # Detaching it (stop-grad through reset depth) is the standard stable-LIF
        # -BPTT trick (snnTorch / Norse 'detach_reset').
        gVh = gV * (1.0 - s)
        if not getattr(args, 'detach_reset', False):
            gs = gs + gV * (-(Vh - V_reset))

        # --- spike (step 4): s = phi(Vh - V_th)*gate ---
        surr_scale = getattr(args, 'surr_scale', 1.0)
        gVh = gVh + gs * surr_scale * spike_surrogate_grad(Vh - V_th, smooth, beta) * gate
        ...
        # --- I_syn (step 1): Isyn = g_e*ge - g_i*gi  (current-based, no V dep) ---
        # d(Isyn)/dV = 0, so NO gV_pre feedback term here: this is exactly what
        # removes the self-amplifying loop that made the conductance adjoint blow up.
        gge_in += gIsyn * syn_gain_exc
        ggi_in += gIsyn * (-syn_gain_inh)
...
def gradient_check():
    ...
    args.syn_gain_exc = 60.0; args.syn_gain_inh = 15.0   # current-based gains
    args.detach_reset = False   # gradcheck tests the TRUE gradient (no detach)
    args.surr_scale = 1.0
...
def train(args):
    ...
    # auto-calibrate background drive so firing sits in target range (per N/seed)
    ...
    np.savez(out,
             ...
             final_mse=final_mse,
             # synapse-model metadata: the benchmark MUST use these to match.
             synapse_model='current', syn_gain_exc=args.syn_gain_exc,
             syn_gain_inh=args.syn_gain_inh)
    print(f'[saved] {out}')
    return out
...
def parse_args():
    ...
    # current-based synapse gains (replace the old (V-E) conductance factors).
    # defaults ~ resting |V-E_exc|=65, |V-E_inh|=15 so drive starts comparable.
    p.add_argument('--syn-gain-exc', type=float, default=60.0)
    p.add_argument('--syn-gain-inh', type=float, default=15.0)
    p.add_argument('--tau-m-min', type=float, default=15.0)
    p.add_argument('--tau-m-max', type=float, default=25.0)
    p.add_argument('--background-mean', type=float, default=3.0,
                   help='initial guess for mean background drive. Auto-calibrated '
                        'before training unless --no-calibrate is set; only the '
                        'per-neuron SPREAD around it is kept after calibration.')
    # firing-rate auto-calibration (frozen-W bisection before training)
    p.add_argument('--no-calibrate', action='store_true',
                   help='skip drive auto-calibration (use --background-mean as-is).')
    p.add_argument('--calib-rate-lo', type=float, default=10.0,
                   help='lower bound of target steady-state firing rate (Hz).')
    p.add_argument('--calib-rate-hi', type=float, default=20.0,
                   help='upper bound of target steady-state firing rate (Hz).')
    p.add_argument('--calib-lo-I', type=float, default=0.0,
                   help='low end of the I_ext-mean bisection search range.')
    p.add_argument('--calib-hi-I', type=float, default=30.0,
                   help='high end of the I_ext-mean bisection search range.')
    p.add_argument('--calib-steps', type=int, default=5000,
                   help='sim-steps per calibration trial. Must comfortably exceed '
                        'the adaptation timescale (tau_w/dt) so the settled tail '
                        'reflects the rate the trainer actually sees.')
    p.add_argument('--calib-warm-frac', type=float, default=0.6,
                   help='fraction of each calibration trial discarded as warmup; '
                        'only the settled tail is counted (kills the adaptation '
    ...
    p.add_argument('--detach-reset', action='store_true', default=True,
                   help='stop-gradient through the reset magnitude (stable-LIF '
                        'BPTT). On by default; this is what keeps the adjoint bounded.')
    p.add_argument('--no-detach-reset', dest='detach_reset', action='store_false',
                   help='backprop through reset (matches true gradient but unstable).')
    p.add_argument('--surr-scale', type=float, default=1.0,
                   help='multiplier on the surrogate spike gradient (lower = '
                        'weaker through-spike gain, more stable).')
    p.add_argument('--lr-w', type=float, default=1e-4)
